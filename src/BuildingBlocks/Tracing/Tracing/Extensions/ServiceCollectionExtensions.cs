...
namespace Tracing
{
    ...
    /// <summary>
    /// Defines the class for ServiceCollectionExtensions
    /// </summary>
    public static class ServiceCollectionExtensions
    {
        ...
        public static IApplicationBuilder UseJaeger(this IApplicationBuilder app)
        {
            ...
            var options = scope.ServiceProvider.GetService<IConfiguration>()?.GetOptions<Options>("Jaeger");
            return options.Enabled ? app.UseMiddleware<Middleware>() : app;
        }

        private static ISampler GetSampler(Options options)
        {
        ...
        private static Options GetJaegerOptions(IServiceCollection services)
        {
            using var seriveProvider = services.BuildServiceProvider();

            var configuration = seriveProvider.GetService<IConfiguration>();

            return configuration.GetOptions<Options>("Jaeger");
        }
    }
}
